...
void avdl_skeleton_SetAnimations(struct avdl_skeleton *o, int animationsCount, struct dd_animation *animations) {
	...
	for (int i = 0; i < o->animations_count; i++) {
		...
		for (int j = 0; j < o->boneCount; j++) {
			...
			// positions
			animBone->keyframe_count_positions = avdl_dynamic_array_count(&animBoneSrc->keyframes_position);
			...
			for (int k = 0; k < animBone->keyframe_count_positions; k++) {
				...
				struct dd_keyframe_vec3 *target = avdl_dynamic_array_get(&animBoneSrc->keyframes_position, k);
			...
			// rotations
			animBone->keyframe_count_rotations = avdl_dynamic_array_count(&animBoneSrc->keyframes_rotation);
			...
			for (int k = 0; k < animBone->keyframe_count_rotations; k++) {
				...
				struct dd_keyframe_vec4 *target = avdl_dynamic_array_get(&animBoneSrc->keyframes_rotation, k);
			...
			avdl_dynamic_array_free(&animBoneSrc->keyframes_position);
			avdl_dynamic_array_free(&animBoneSrc->keyframes_rotation);
		}
